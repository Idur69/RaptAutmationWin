...
from RaptAutomation.Src.EnvSetup.cnfgurl import EnvironmentSetup, LoginUsers, Paths
from RaptAutomation.Src.PageObject.Pages.MyUrl import Myurl
from RaptAutomation.Test.TestUtility.ScreenShots import SS


class Kubernetes_Ui_Parallel_Pgan_Mnist_04(EnvironmentSetup):
    ...
    def mnist_auto(self):
        ...
        # inception = self.driver2.find_element_by_css_selector('.card-body text-center font-weight-normal btnNext')
        # inception = self.driver2.find_element(By.LINK_TEXT, 'Inception')
        ...
        localpath.send_keys(locpath)
        print("Your given path tensorflow-UI")
        # ----------GPU Auto --------
        ...
        print("Your selected Auto")
        # ------Screenshot-1-----------
        ss.ScreenShot(ss_path + "test_04_singlGpu_mnist_auto_setupscreen2.png")
        # -------------------- setup btn -----------------
        ...
        sleep(1)
        # --------Screen shot-2 -----------
        ss.ScreenShot(ss_path + "test_04_singlGpu_mnist_auto_elapsedtime2.png")
        sleep(2)
        assert isinstance(myElem.text, object)
        print("Mnist Auto-1 -", str(myElem.text))
    ...
    def test_pagan_auto_mnist_auto(self):
        u1 = multiprocessing.Process(target=self.pgan_auto)
        u2 = multiprocessing.Process(target=self.mnist_auto)

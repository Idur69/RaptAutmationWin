...
class Kubernetes_Cli_Nfs_Pgan_Manual(unittest.TestCase):
    ...
    def test_nfs_pgan_manual(self):
        ...
        nfspgan_cmd = "raptrun train"+" -name " + name +" -f " + fw +" -a " + application +" -d " + dirpath +" -cf "+ config +" -t " + training +" -type " + type1 +" -l " + location +" -mode " + mode +" -p " + memoryper +"-cp " + coreper +" -nfsip " +nfsip + " -nfspath " + nfspath     
         
        #cmd = "raptrun -name demotwo -f tensorflow -a pggan -d /home/ubuntu/tensorflow-UI/ -t /tensorflow/training/pgan/train.py -cf /tensorflow/training/pgan/config.py -type rapt -l local -mode manual -p 40 -cp 90"

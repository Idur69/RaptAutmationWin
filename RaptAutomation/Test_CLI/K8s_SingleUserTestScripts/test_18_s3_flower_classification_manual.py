...
class Kubernetes_Cli_S3_Image_Manual(unittest.TestCase):
    ...
    def test_s3_image_manual(self):
       ...
       # config = path.s3_pgan_config
        ...
        #cmd = 'raptrun -name demotwo -f tensorflow -a flower_classification -d /s3bucket/rapt -t /tensorflow/tensorflow-UI-less/training/flower_classification/retrain-new.py -type rapt -l s3bucket -mode manual -p 50 -cp 90 -k AKIAITOV7UZRONOPBIHA:PmlQsH2tg7+7jexDMPzN0VGat/9VPa6BIapcAPYe -bucket rapt-data-bucket -m /tensorflow/tensorflow-UI-less/inception -i /tensorflow/tensorflow-UI-less/datasets/flower_photos -b /tensorflow/tensorflow-UI-less/datasets/bottlenecks-new -s 50 '
        s3image_cmd = "raptrun train"+" -name " + name+ " -f " + fw +" -a " + application +" -d " + dirpath +" -t " + training +" -type " + type1+" -l " + location +" -mode " + mode +" -p " + memoryper +"-cp " + coreper +" -k " + bkeys +" -bucket " + bname +" -m " + modeldir+" -i " + imagedir +" -b " + bottleneck +" -s " + steps

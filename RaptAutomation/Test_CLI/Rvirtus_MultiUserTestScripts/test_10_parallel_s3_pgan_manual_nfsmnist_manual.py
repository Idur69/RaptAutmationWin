...
class Rvirtus_Multiusers_10(unittest.TestCase):
    ...
    def run_nvidia_smi(self, nvidiasmi):
        ...
        print(nvidia)
    def test_parallel_s3pganmanual_nfsmnistmanual01(self):
        ...
        s3_cmd = "/home/ubuntu/test/raptcli raptrun"+" -name "+ name +" -f " + fw +" -a " +application +" -d " + dirpath +" -cf " + config +" -t " + training +" -type " + type1 +" -l " + location +" -mode " + mode +" -p " + memoryper +" -cp " + coreper +" -k " + bkeys +" -bucket " + bname + " -pf " + platform
        ...
        #s3_cmd = "raptrun -name demo1 -f tensorflow -a pggan -d /home/ubuntu/tensorflow-UI/ -t /tensorflow/tensorflow-UI-less/training/pgan/train.py -cf /tensorflow/tensorflow-UI-less/training/pgan/config.py -type rapt -l s3bucket -mode manual -p 50 -cp 90 -k AKIAITOV7UZRONOPBIHA:PmlQsH2tg7+7jexDMPzN0VGat/9VPa6BIapcAPYe -bucket rapt-data-bucket"

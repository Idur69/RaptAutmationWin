...
class Rvirtus_Cli_Nfs_Image_Auto(unittest.TestCase):
    ...
    def run_cmd1(self, nfsimage_cmd):
        ...
        print("&&&&&&&&&&---Yor command-1 ---&&&&&&&&&&&" + str(nfsimage_cmd))
    ...
    def test_nfs_image_auto(self):
        ...
        #cmd = "raptrun -name demotwo -f tensorflow -a flower_classification -t /tensorflow/training/flower_classification/retrain-new.py -type rapt -l nfs -mode manual -p 50 -cp 90 -m /tensorflow/inception -i /tensorflow/datasets/flower_photos4.5GB -b /tensorflow/datasets/bottlenecks4.5GB-new -s 50 -nfsip 52.34.222.154 -nfspath /home/ubuntu/tensorflow-UI -pf raptvirtus"
        nfsimage_cmd = "raptrun"+" -name " + name+ " -f " + fw +" -a " + application +" -t " + training +" -type " + type1+" -l " + location +" -mode " + mode +" -m " + modeldir+" -i " + imagedir +" -b " + bottleneck +" -s " + steps +" -nfsip " +nfsip + " -nfspath " + nfspath + " -pf " + platform

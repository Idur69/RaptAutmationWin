...
class Rvirtus_Cli_Image_Auto(unittest.TestCase):
    ...
    def test_local_image_auto(self):
       ...
       # raptrun = cmddata.raptrun
        ...
        image_cmd = "raptrun "+" -name " + name+ " -f " + fw +" -a " + application +" -d " + dirpath +" -t " + training +" -type " + type1+" -l " + location +" -mode " + mode + " -m " + modeldir+" -i " + imagedir +" -b " + bottleneck +" -s " + steps + " -pf " + platform
        #cmd = "raptrun -name demotwo -f tensorflow -a flower_classification -d /home/ubuntu/tensorflow-UI  -t /tensorflow/training/flower_classification/retrain-new.py -type rapt -l local  -mode manual -p 50 -cp 90 -m /tensorflow/inception -i /tensorflow/datasets/flower_photos4.5GB -b /tensorflow/datasets/bottlenecks4.5GB-new -s 50 -pf raptvirtus"

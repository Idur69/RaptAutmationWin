...
class Rvirtus_Cli_Nfs_Mnist_Manual(unittest.TestCase):
    ...
    def test_nfs_mnist_manual(self): 
        ...
        #nfsmnist_cmd = "raptrun -name demotwo -f tensorflow -a mnist -d /home/ubuntu/tensorflow-UI/ -t /tensorflow/training/Mnist_classification/mnist_deep.py -type rapt -l nfs -mode manual -p 50 -cp 90 -nfsip 34.221.21.217 -nfspath /home/ubuntu/tensorflow-UI -pf raptvirtus"
        nfsmnist_cmd = "raptrun"+" -name " + name +" -f " + fw +" -a " + application +" -d " + dirpath +" -t " + training +" -type " + type1 +" -l " + location +" -mode " + mode +" -p " + memoryper +" -cp " + coreper +" -nfsip " +nfsip + " -nfspath " + nfspath + " -pf " + platform
        ...
        #signal_err = "Command terminated by signal 11"
        #if self.assertTrue(regexp):
            #print("Error True")
        #else:
            #assert print("Error Occured.... !")
        #if self.assertTrue(signal_err):
            #print("Singnal command true!.....")
        #else:
            #assert print("Signal Error !" + signal_err)

...
class Rvirtus_Cli_S3_Mnist_Auto(unittest.TestCase):
    ...
    def test_s3_mnist_auto(self):
       ...
       # raptrun = cmddata.raptrun
        ...
        #cmd = "raptrun -name demotwo -f tensorflow -a mnist -d /home/ubuntu/tensorflow-UI/ -t /tensorflow/tensorflow-UI-less/training/Mnist_classification/mnist_gpu.py -type rapt -l s3bucket -mode manual -p 70 -cp 90 -k AKIAITOV7UZRONOPBIHA:PmlQsH2tg7+7jexDMPzN0VGat/9VPa6BIapcAPYe -bucket rapt-data-bucket -pf raptvirtus "
        s3mnist_cmd = "raptrun"+" -name "+ name +" -f " + fw +" -a " +application +" -d " + dirpath +" -t " + training +" -type " + type1 +" -l " + location +" -mode " + mode +" -k " + bkeys +" -bucket " + bname + " -pf " + platform
